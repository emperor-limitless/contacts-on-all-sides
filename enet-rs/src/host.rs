...
impl<T> Host<T> {
    ...
    pub fn peers(&'_ mut self) -> impl Iterator<Item = Peer> {
    ...
    /// performance.
    pub fn service(&'_ mut self, timeout_ms: u32) -> Result<Option<Event>, Error> {
    ...
    /// available
    pub fn check_events(&'_ mut self) -> Result<Option<Event>, Error> {
        // ENetEvent is Copy (aka has no Drop impl), so we don't have to make sure we
        // `mem::forget` it later on
        let mut sys_event = MaybeUninit::uninit();
